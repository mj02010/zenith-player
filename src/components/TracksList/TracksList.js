	...
	const tracksListElems = tracksList.map((track, index) => {
		return (
			<li
				key={index}
				isFavorite={
					track.isFavorite && track.isFavorite === true
						? track.isFavorite.toString()
						: 'false'
				}
				index={index}
				data-trackid={index}
				className={index === props.currentTrackID ? 'active' : ''}
				id={index === props.currentTrackID ? 'activeTrack' : ''}
				style={{
					display:
						(props.favsonly && !track.isFavorite) ||
						(props.searchString !== '' &&
							!track.title
								.toLowerCase()
				...
				onClick={(ev) => props.selectThisTrack(ev, index)}
			>
				<span>
					<strong>{index === props.currentTrackID ? '> ' : ''}</strong>
					{track.title}
				</span>
			</li>
		);
		...
		>
			...
			<div className="bottom-ctr">
				...
				<div className="bottom-btns-ctr">
					...
					></button>
					<button
						aria-label="favorites filter button"
						className="favs-btn"
						dangerouslySetInnerHTML={{
							__html: props.favsonly

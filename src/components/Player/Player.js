...
import { tracks } from '../../libs/tracks.json';
...
class Player extends Component {
	constructor(props) {
		...
		this.toggleFavorite = () => {
			...
			this.state.tracksList[this.state.trackID].isFavorite === true
				? (this.state.tracksList[this.state.trackID].isFavorite = true)
				: (this.state.tracksList[this.state.trackID].isFavorite = false);
		};

		this.toggleShowfavsonly = () => {
		...
		this.selectThisTrack = (event, trackID) => {
			let currentTrackIndex = 0;
			if (this.state.tracksList[trackID]) currentTrackIndex = trackID;
			this.setState({ currentTrackIndex: currentTrackIndex }, () => {
				this.loadTrackIntoState();
			});
			return false;
		};
		...
		this.updateProgressBar = () => {
			const progressHandleCtrParent =
				document.getElementById('progressHandleCtr').parentElement;
		...
		this.onPlayPauseClickHandler = (event) => {
			this.setState({ playing: !this.state.playing }, () => {
				this.playPauseAudio();
			});
		};

		this.touchPositionFst = 0;
		this.spannedTime = 0;
		this.volume = 0;

		this.progBarPickHandler = (event) => {
			...
			const progressBarWidth = parseFloat(
				window.getComputedStyle(handleCtr.parentElement, null).width,
			);

			let handleCtrStyleLeft =
				this.touchPositionFst -
				handleCtr.parentElement.clientLeft -
				parseFloat(
		...
		this.volumeBarPickHandler = (event) => {
			if (event.type === 'mousedown') {
				this.touchPositionFst = event.pageX;
		...
		this.volumeBarMoveHandler = (event) => {
			let touchPositionSnd = 0;
			if (event.type === 'mousemove') {
				touchPositionSnd = event.pageX;
				document.addEventListener('mouseup', this.volumeBarDropHandler);
		...
		this.volumeBarDropHandler = (event) => {
			this.setState(
				{
					volumeHandleActive: false,
				},
		...
		this.loadTrackIntoState = () => {
			const trackURL = `https://${window.location.host}/audio/${this.state.currentTrackIndex}.mp3`;
			const trackTitle =
			...
			this.setState(
				{
					trackID: this.state.currentTrackIndex,
					trackURL: trackURL,
					trackTitle: trackTitle,
					isFavorite:
						this.state.tracksList[this.state.currentTrackIndex].isFavorite,
					player: player,
					trackLoaded: false,
					playing: false,
					...
					volumeHandleCtrStyleLeft:
						player.volume *
							parseFloat(
								window.getComputedStyle(
									document.getElementById('volumeBarCtr'),
									null,
								).width,
							) +
						'px',
			...
			<div className="Player">
				...
				<div className="ctr-1">
					<p className="play-pause-text">
						<strong>{this.state.playing ? 'PLAYING' : 'PAUSED'}</strong>
						<span>
				...
				<div className="bottom-ctr">
					...
					<div className="control-icons-container">
						<button
							...
							dangerouslySetInnerHTML={{
								__html: `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 70.7 70.7" style="enable-background:new 0 0 70.7 70.7;" xml:space="preserve"><g><rect x="3.2" y="20.5" width="8.9" height="29.7"/><polygon points="59.4,54.3 23,35.3 59.4,16.3"/></g></svg>`,
							}}
						...
						<button
							...
							dangerouslySetInnerHTML={{
								__html: this.state.playing
									? `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 80 80" style="enable-background:new 0 0 80 80;" xml:space="preserve"><g><rect x="15.7" y="9.2" width="19.2" height="61.7"/><rect x="45.2" y="9.2" width="19.2" height="61.7"/></g></svg>`
						...
						<button
							...
							dangerouslySetInnerHTML={{
								__html: `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 70.7 70.7" style="enable-background:new 0 0 70.7 70.7;" xml:space="preserve"><g><rect x="58.5" y="20.5" width="8.9" height="29.7"/><polygon points="11.3,54.3 47.7,35.3 11.3,16.3"/></g>`,
							}}
					...
					<div className="bottom-icons-ctr">
						<button
							...
							dangerouslySetInnerHTML={{
								__html: `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 225 110" style="enable-background:new 0 0 225 110;" xml:space="preserve"><path d="M224.2,55c0,5-4,9-9,9H9.8c-5,0-9-4-9-9s4-9,9-9h205.3C220.1,46,224.2,50,224.2,55z M180.5,83.8H9.8c-5,0-9,4-9,9s4,9,9,9 h170.6c5,0,9-4,9-9S185.4,83.8,180.5,83.8z M9.8,26.2h135.7c5,0,9-4,9-9s-4-9-9-9H9.8c-5,0-9,4-9,9S4.9,26.2,9.8,26.2z"/></svg>`,
							}}
						...
						<button
							...
							dangerouslySetInnerHTML={{
								__html: `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 225 110" style="enable-background:new 0 0 225 110;" xml:space="preserve"><path d="M224.2,66.5c0,23.6-19.2,42.8-42.8,42.8H43.6C20,109.3,0.8,90.1,0.8,66.5S20,23.7,43.6,23.7h30c5,0,9,4,9,9s-4,9-9,9h-30 c-13.7,0-24.8,11.1-24.8,24.8S30,91.3,43.6,91.3h137.7c13.7,0,24.8-11.1,24.8-24.8S195,41.7,181.4,41.7h-39.9l8.1,8.1 c3.5,3.5,3.5,9.2,0,12.7c-1.8,1.8-4.1,2.6-6.4,2.6s-4.6-0.9-6.4-2.6L113.3,39c-3.5-3.5-3.5-9.2,0-12.7l22.9-22.9 c3.5-3.5,9.2-3.5,12.7,0c3.5,3.5,3.5,9.2,0,12.7l-7.6,7.6c0,0,0,0,0,0h40C205,23.7,224.2,42.9,224.2,66.5z"/></svg>`,
							}}
